...
def get_cpgs(chromosome: str = None,
             upstream: int = 0,
             downstream: int = 0) -> IntervalFrame:
    """
    Get CpGs
    ...
    # Find file
    hg19_h5 = get_data_file("hg19_CpGs.h5")
    f = h5py.File(hg19_h5, "r")
    ...
    if chromosome is None:
        for chrom in list(f.keys()):
            if chrom != "n_CpGs":
                starts = np.array(f[chrom])
                ends = starts + 2
                chroms = np.repeat(chrom, len(starts))
                intervals.add(starts, ends, chroms)
    else:
        starts = np.array(f[chromosome])
        ends = starts + 2
        chroms = np.repeat(chromosome, len(starts))
        intervals.add(starts, ends, chroms)
    ...
    # Construct IntervalFrame
    iframe = IntervalFrame(intervals=intervals)
...
def get_cpg_names() -> IntervalFrame:
    """
    Get CpG names
    ...
    # Construct
    cpgs = np.array([]).astype(str)
    for chrom in list(f.keys()):
        if chrom != "n_CpGs":
            starts = np.array(["_"+s for s in np.array(f[chrom]).astype(str)])
            chroms = np.repeat(chrom, len(starts)).astype(str)
            values = np.core.defchararray.add(chroms, starts)
            cpgs = np.append(cpgs, values)
